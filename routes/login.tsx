    ...
    async POST(req, res) {
        ...
        if (!email || !password) {
            return new Response("Missing Email or Response", {
                status: 301,
                headers: {
                    Location: `/login?data=${encodeURIComponent(
                        JSON.stringify({
        ...
        if (!user) {
            return new Response("User doesn't exist", {
                status: 301,
                headers: {
                    Location: `/login?data=${encodeURIComponent(JSON.stringify({ error: "User doesn't exist" }))}`,
                },
            });
        }
        ...
        <html lang="en">
            ...
            <body class="font-forum">
                <section class="flex flex-wrap space-x-3 space-y-4 pt-28 pb-4 justify-around items-center inset-0 bg-[linear-gradient(to_right_bottom,rgba(0,0,0,0.3),rgba(0,0,0,0.3)),url('https://imagescdn.homes.com/i2/bRuQxp-5oG4CrZgJ0vb1z8OnBtQoY9k3yIt0m5imaRc/117/gann-academy-waltham-ma-primaryphoto.jpg?p=1')] bg-cover bg-center backdrop-blur-sm">
                    <form method="POST" class="w-[60%] max-md:w-[80%] p-4 bg-white flex flex-col justify-center items-center rounded-[2rem]">
                        <h1 class="my-4 uppercase text-red-900 text-shadow-mdblack tracking-wide text-[4rem] text-center">Log in</h1>
                        <input placeholder="Email" class="border-none focus:outline-none shadow-md my-4 p-4 text-[2rem] max-sm:text-[1.25rem] rounded-2xl w-[70%] max-sm:w-[80%] bg-slate-200" type="email" name="email" required />
                        <input placeholder="Password" class="border-none focus:outline-none shadow-md my-8 p-4 text-[2rem] max-sm:text-[1.25rem] rounded-2xl w-[70%] max-sm:w-[80%] bg-slate-200" type="password" name="password" required />
                        {data.error ? <p class="text-red-900">Error: {data.error}</p> : <p></p>}
                        <p class="my-4 text-[1.25rem] max-sm:text-[1rem]">
                            Don't have an account yet?{" "}
                            <a class="text-red-900" href="/signup">
                                Signup!
                            </a>
                        </p>
                        <button class="bg-red-900 px-8 py-4 text-[2rem] rounded-[2.5rem] text-white hover:translate-y-2 transition-all hover:shadow-md hover:shadow-red-950" type="submit">
                            Submit
                        </button>
                        ...
                        <a href="/auth/login" class={"bg-red-900 p-4 text-white rounded-3xl text-2xl transition-all mb-4 text-center hover:translate-y-1 flex space-x-3"}>
