    ...
    async POST(req, res) {
        ...
        if (!isAlphanumeric(username!) || !isLength(username!, { min: 4, max: 20 })) {
            return Response.json(null, {
                status: 303,
                headers: {
                    Location: `/signup?data=${encodeURIComponent(
                        JSON.stringify({
                            email: email,
                            username: username,
                            password: password,
                            error: "The username must be between lengths 4-20, and the username should be alphanumeric",
                        }),
                    )}`,
                },
            });
        ...
        if ((await findUserByEmail(email)) || (await kv.get(["usernames", username])).value) {
        ...
        <html lang="en">
            ...
            <body class="font-forum">
                <section class="flex h-[100vh] justify-center items-center flex-wrap space-x-3 space-y-4 pt-28 pb-4 inset-0 bg-[linear-gradient(to_right_bottom,rgba(0,0,0,0.3),rgba(0,0,0,0.3)),url('https://imagescdn.homes.com/i2/bRuQxp-5oG4CrZgJ0vb1z8OnBtQoY9k3yIt0m5imaRc/117/gann-academy-waltham-ma-primaryphoto.jpg?p=1')] bg-cover bg-center backdrop-blur-sm">
                    <form action="/api/signup" method="POST" class="w-[25rem] p-4 h-[33rem] bg-white justify-center items-center flex-col flex max-sm:w-[20rem]">
                        <h1 class="uppercase tracking-wide text-[2.5rem] text-center text-shadow-mdblack text-red-900 my-2 max-sm:text-[2rem]">Two Step Authentication</h1>
                        <input type="hidden" name="email" value={data.email} />
                        <input type="hidden" name="username" value={data.username} />
                        <input type="hidden" name="password" value={data.password} />
                        <p class="text-center my-4">We've just sent a code to your email, please enter the code here.</p>
                        <input class="border-none focus:outline-none shadow-md my-8 p-4 text-[2rem] w-[70%] max-sm:w-[80%] bg-slate-200 tracking-widest" type="text" name="code" maxlength={6} />
                        <button type="submit" class="bg-red-900 signup-submit p-4 text-[2rem] uppercase tracking-wide text-white shadow-black/50 shadow-md w-[50%] rounded-[2rem] transition-all hover:translate-y-2 mb-8">
                            Submit
                        </button>
