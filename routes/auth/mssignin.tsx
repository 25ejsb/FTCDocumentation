...
import { createSession, createUser, findUserByEmail, kv, User } from "../../utils/db.ts";
...
export const handler: Handlers = {
    async GET(req, ctx) {
        ...
        try {
            ...
            return new Response("Authentication callback failed", {
                status: 500,
            });
        }
    },
    ...
    return (
        <html lang="en">
            <head>
                ...
                <title>Enter a Username - FTC Gann Documentation</title>
            </head>
            <body class="font-forum">
                <section class="flex flex-wrap space-x-3 space-y-4 pt-28 pb-4 justify-around items-center inset-0 bg-[linear-gradient(to_right_bottom,rgba(0,0,0,0.3),rgba(0,0,0,0.3)),url('https://imagescdn.homes.com/i2/bRuQxp-5oG4CrZgJ0vb1z8OnBtQoY9k3yIt0m5imaRc/117/gann-academy-waltham-ma-primaryphoto.jpg?p=1')] bg-cover bg-center backdrop-blur-sm">
                    <form method="POST" class="w-[60%] relative max-md:w-[80%] p-4 bg-white flex flex-col justify-center items-center rounded-[2rem]">
                        <h1 class="my-4 uppercase text-red-900 text-shadow-mdblack tracking-wide text-[4rem] text-center">Enter Username</h1>
                        <input placeholder="Username" class="border-none focus:outline-none shadow-md my-8 p-4 text-[2rem] max-sm:text-[1.25rem] rounded-2xl w-[70%] max-sm:w-[80%] bg-slate-200" type="text" name="username" minlength={4} maxlength={20} required />
